...
instance Functor ListZipper where
  f <$> ListZipper l a r = ListZipper (map f l) (f a) (map f r)
...
hasRight ::
  ListZipper a
  -> Bool
hasRight l = not $ isEmpty (rights l)
...
findLeft p (ListZipper l a r)
  | (x :. xs) <- r' = IsZ $ ListZipper xs x (reverse l' ++ a :. r)
  | otherwise       = IsNotZ
  where (l', r') = break p l
...
findRight p (ListZipper l a r)
  | (x :. xs) <- r' = IsZ $ ListZipper (reverse l' ++ a :. l) x xs
  | otherwise       = IsNotZ
  where (l', r') = break p r
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (l :. ls) a r) = IsZ (ListZipper ls l (a :. r))
moveLeft _                          = IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l a (r :. rs)) = IsZ (ListZipper (a :. l) r rs)
moveRight _                          = IsNotZ
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (l :. ls) a r) = IsZ (ListZipper (a :. ls) l r)
swapLeft _                          = IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper l a (r :. rs)) = IsZ (ListZipper l r (a :. rs))
swapRight _                          = IsNotZ
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN n l
  | n > 0     = fromOptional $ toOptional (moveLeft l) >>= toOptional . moveLeftN (n-1)
  | n == 0    = IsZ l
  | otherwise = moveRightN (-n) l
...
moveRightN ::
  ...
  -> MaybeListZipper a
moveRightN n l
  | n > 0     = fromOptional $ toOptional (moveRight l) >>= toOptional . moveRightN (n-1)
  | n == 0    = IsZ l
  | otherwise = moveLeftN (-n) l
...
moveLeftN' n l@(ListZipper ls _ rs) = case (moveLeftN n l) of
  IsNotZ -> if n > 0 then Left (length ls) else Left (length rs)
  IsZ x  -> Right x
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n l@(ListZipper ls _ rs) = case (moveRightN n l) of
  IsNotZ -> if n > 0 then Left (length rs) else Left (length ls)
  IsZ x  -> Right x
...
nth n lz@(ListZipper l _ _)
  | n >= nl   = moveRightN (n - nl) lz
  | otherwise = moveLeftN (nl - n) lz
  where nl = length l
...
end (ListZipper l x r)     = let (a :. as) = reverse r
                             in ListZipper (as ++ x :. l) a Nil
...
start (ListZipper l x r)     = let (a :. as) = reverse l
                               in ListZipper Nil a (as ++ x :. r)
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft v (ListZipper l x r) = ListZipper (x :. l) v r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight v (ListZipper l x r) = ListZipper l v (x :. r)
...
instance Extend ListZipper where
  f <<= l = ListZipper (unfoldr (f' . moveLeft) l) (f l) (unfoldr (f' . moveRight) l)
