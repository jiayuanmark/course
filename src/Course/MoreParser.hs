...
tok ::
  Parser a
  -> Parser a
tok p = (\x -> (spaces >>> valueParser x)) =<< p
...
hex ::
  Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>= (\x -> let (Full c) = readHex x
                                                   in pure (chr c))
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 p sep = p >>= (\a -> list (sep >>> p) >>= (\as -> pure (a :. as)))
...
eof ::
  Parser ()
eof = P $ \s -> if isEmpty s then Result s () else ErrorResult (ExpectedEof s)
