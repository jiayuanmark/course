...
character ::
  Parser Char
character = P p
  where p (c :. cs) = Result cs c
        p Nil       = ErrorResult Failed
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f pa = P $ \s -> case (parse pa s) of
  Result rst a  -> Result rst (f a)
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f pa = P $ \s -> case (parse pa s) of
  ErrorResult e -> ErrorResult e
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa = bindParser (\a -> mapParser (a :.) (list pa)) pa
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = flbindParser character $ \c -> if p c
                                           then (valueParser c)
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) $ \x -> let Full v = read x
                                             in valueParser v
--
...
sequenceParser = foldRight f (valueParser Nil)
  where f p ps = flbindParser p (\x -> mapParser (x :. ) ps)
...
firstNameParser ::
  Parser Chars
firstNameParser = flbindParser upper $ \x ->
                  flbindParser (list lower) $ \y ->
                  valueParser (x :. y)
...
surnameParser ::
  Parser Chars
surnameParser = flbindParser upper $ \x ->
                flbindParser (thisMany 5 lower) $ \y ->
                flbindParser (list lower) $ \z ->
                valueParser (x :. y ++ z)
...
phoneParser ::
  Parser Chars
phoneParser = flbindParser digit $ \x ->
              flbindParser phoneBodyParser $ \y ->
              flbindParser (is '#') $ \_ ->
              valueParser $ x :. y
...
personParser ::
  Parser Person
personParser = flbindParser ageParser $ \ag ->
               flbindParser (spaces1 >>> firstNameParser) $ \fn ->
               flbindParser (spaces1 >>> surnameParser) $ \sn ->
               flbindParser (spaces1 >>> smokerParser) $ \smk ->
               flbindParser (spaces1 >>> phoneParser) $ \phn ->
               valueParser $ Person ag fn sn smk phn
