...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (<*>) f g = State $ \s -> let (h, s1) = runState f s
                                (a, s2) = runState g s1
                            in  (h a, s2)
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  (=<<) f g = State $ \s -> let (a, s1) = runState g s
                            in runState (f a) s1
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat l = eval (findM p l) S.empty
  where p a = State $ \s -> (a `S.member` s, S.insert a s)
...
distinct l = eval (filtering p l) S.empty
  where p a = State $ \s -> (not $ S.member a s, S.insert a s)
...
isHappy ::
  Integer
  -> Bool
isHappy x   = contains 1 $ eval (findM p l) S.empty
  where g   = P.fromIntegral . P.sum . P.map (P.^2) . P.map digitToInt . show
        l   = produce g x
        p a = State $ \s -> (a == 1 || a `S.member` s, S.insert a s)
