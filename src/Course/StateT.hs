...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f sa = StateT $ \s -> let v        = runStateT sa s
                                  f' (a,b) = (f a, b)
                              in f' <$> v
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) sf sa = StateT $ \s -> let f = runStateT sf s
                               in f >>= \(f',s1) -> (runStateT sa s1 >>= \(a,s2) -> pure (f' a,s2))
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) f sa = StateT $ \s -> let a = runStateT sa s
                              in a >>= \(a', s')-> runStateT (f a') s'
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  (<*>) f a = OptionalT $ (lift2 ($)) <$> (runOptionalT f) <*> (runOptionalT a)
...
distinctG l = runOptionalT $ evalT (filtering p l) S.empty
